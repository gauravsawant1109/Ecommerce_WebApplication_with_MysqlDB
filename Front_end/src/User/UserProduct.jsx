...
//   const [brands, setBrands] = useState([]);
//   const [ShowFilterModal, setShowFilterModal] = useState(false);
//   const [filteredData, setFilteredData] = useState({ selectedCategory: null, selectedBrand: null });
...
//   useEffect(() => {
//     async function fetchData() {
//       const ApiResponse1 = await axios.get("http://localhost:5000/product/getAllProduct");
//       setProducts(ApiResponse1.data.allProducts);

//       const categoryResponse = await axios.get("http://localhost:5000/category/getAllCategories");
//       setCategories(categoryResponse.data.categoriesResult);

//       const brandResponse = await axios.get("http://localhost:5000/brand/getAllBrand");
//       setBrands(brandResponse.data.brands);

//       const ApiResponse2 = await axios.get("http://localhost:5000/product/CountOfProduct");
//       setProductCount(ApiResponse2.data.ProductCount.TotalProduct);
//     }
...
//           <div className="d-flex justify-content-center">
//             {categories.map((c) => (
//               <Card key={c.category_id} className="m-2" style={{ width: "15rem" }} onClick={() => navigate(`/UserHome/SelectedCategory/${c.category_id}`)}>
//                 <Card.Img variant="top" src={`http://localhost:5000/uploads/${c.category_image}`} style={{ height: "12rem" }} alt="Category image" />
//                 <Card.Body className="p-0 text-center">
//                   <Card.Title>{c.category_name}</Card.Title>
//                 </Card.Body>
//               </Card>
...
//       <div className="d-flex text-end fs-5">
//         <p className="fw-bold m-2 pe-2 w-100">Total Products: {ProductCount}</p>
//         <div className="m-1 text-primary" onClick={() => setShowFilterModal(true)}><FaFilter /></div>
//       </div>
...
//         <div className="d-flex flex-wrap justify-content-center align-items-center pb-4">
//        {Products.map((product) => (
//          <div key={product.product_id} className="card cu-card m-2 border border-1" style={{ width: "250px", height: "395px" }}>
//            <img src={`http://localhost:5000/uploads/${product.product_image}`} className="card-img-top" alt="Product Image" style={{ height: "300px" }} />
//            <div className="card-body p-2">
...
//          </div>
//        ))}
//      </div> : <h1 className="text-center">Product Not Found</h1>
//      }

//       {ShowFilterModal && <ProductFilterModule Show={ShowFilterModal} handleClose={() => setShowFilterModal(false)} setFilteredData={setFilteredData} />}
//     </>
//   );
// };
