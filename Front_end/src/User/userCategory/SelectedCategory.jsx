...
const navigate = useNavigate();
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/product/getProductByCategory/${category_id}`
        );
        setSelectedProducts(response.data.allProducts);
        setSelectedCategory(response.data.category.category_name);
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-center mb-4 ">Category : {selectedCategory} </h2>
